<template>
  <div id="app" class="h-full flex flex-col" v-if="! loading">
    ...
    <div class="bg-gray-200 border-t border-gray-400 p-2 text-sm flex justify-between">
      ...
      <p><span class="cursor-pointer" @click="refreshIP">{{ refreshing ? 'Refreshing…' : '⟳ Your ip' }}</span>: <span class="select-all cursor-pointer hover:text-primary" @click="copyIP">{{ $store.ip }}</span></p>
